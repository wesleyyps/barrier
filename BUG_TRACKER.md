...
| 2026-07-26 | DEPRECATION_ERADICATION | `Log.cpp`, `ServerConfig.cpp`, `SecureUtils.cpp`, GUI tests | 46f684c4 | Fixed macOS 13 SDK (`sprintf`->`snprintf`), Qt 5.15 (`endl`, `MidButton`, `QFlags`, `pixmap`), OpenSSL 3.0 (`RSA_generate_key`), and GoogleTest (`Invoke`) deprecations. | **COMMITTED** |
| 2026-07-26 | FINAL_MODERNIZATION | Across codebase (61 files) | 0d9ed37c | Automated fixes for `use-equals-default`, `implicit-bool-conversion`, `make-member-function-const`, `redundant-void-arg`, and `redundant-smartptr-get`. Reverted `ServerApp` to fix template breakage. | **COMMITTED** |
